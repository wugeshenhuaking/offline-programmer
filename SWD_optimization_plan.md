...
### 7.1 为什么修改前 `DAP_DEFAULT_SWJ_CLOCK = 10MHz` 不能用，修改后却可以？

**原因不是“目标芯片不支持 10MHz”**，而是修改前的 **slow 模式实现效率太低**：

- `clock_delay` 只决定“软件插入多少空转”
- 但旧实现每个 SWD bit 都要经过：
  - 多次 GPIO 寄存器访问
  - `gpio_init()` 切换 SWDIO 方向
  - 逐位循环发送/校验
- 所以把名义时钟从 `1MHz` 改到 `10MHz` 后，真实波形并没有变成稳定的 10MHz，反而变成了：
  - 时序余量更小
  - 上升下降沿更差
  - turnaround 阶段更容易错过 ACK

结果就是：**1MHz 配置还能勉强工作，10MHz 配置就会在 SWD 初始化阶段失败。**

修改后 fast 路径做了三件关键事：

- `SWDIO` 改为直接寄存器切换方向
- 位操作改为原子置位/清位
- `SWDIO` 输出改为推挽，提高高速边沿质量

所以这时把时钟设到 `10MHz`，才第一次变成“真实且可工作的高速模式”。

### 7.2 既然 fast 模式更快，为什么还要保留 slow 模式？

因为 **fast 模式不是所有硬件场景都稳定**。

slow 模式仍然有价值：

- 目标板连线较长、干扰较大时，slow 模式更稳
- 某些目标芯片或个体在高 SWCLK 下兼容性更差
- 调试底层 SWD 问题时，slow 模式更容易定位时序问题
- fast 模式失败时，slow 模式可以作为降级和兜底路径

所以更合理的做法不是“只留 fast”，而是：

- **默认优先 fast**
- **保留 slow 作为兼容/排障模式**

### 7.3 为什么 92KB bin，10MHz 下 slow 约 8s，fast 约 6s，差距没有想象中大？

这是正常现象。

**SWD 提速，只能缩短“数据传输”这部分时间**，但一次完整烧录不只有传输，还包括：
...
这些步骤很多都受 **目标 MCU 内部 Flash 操作时间** 限制，不会随着 SWD 线速同比变快。
...
说明当前瓶颈已经不再主要是“GPIO 模拟 SWD 太慢”，而更多转移到了：

- 目标 Flash 擦写时间
- 算法执行与校验时间

也就是说，**fast 模式已经把“链路传输”这部分明显压下去了，但整机烧录时间不可能按线速倍数同比下降。**
