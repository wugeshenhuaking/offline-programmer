...
// STM32 风格 (3个字段): {pin, speed, mode}
// AT32 风格 (4个字段):   {pin, mode, out_type, drive_strength}
...
// SWD 引脚: 推挽输出, 50MHz, 无上下拉
 ...
 gpio_init_type INIT_SWCLK_PINS = {
    PIN_SWCLK_TCK,
    GPIO_OUTPUT_PUSH_PULL,
    GPIO_PULL_NONE,
    GPIO_MODE_OUTPUT,
...
void PORT_SWD_SETUP(void)
{
    ...
    PIN_SWCLK_TCK_PORT->clr = (PIN_SWCLK_TCK);
    ...
    for (; request_count; request_count--)
    {
        ...
        if (request_value & DAP_TRANSFER_RnW)
        {
            ...
            if (request_value & DAP_TRANSFER_MATCH_VALUE)
            {
                ...
                match_value =
                    (*(request + 0) << 0) | (*(request + 1) << 8) | (*(request + 2) << 16) | (*(request + 3) << 24);
                request += 4;
